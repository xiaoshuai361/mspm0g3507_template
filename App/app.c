...
static void App_Task2Reset(uint32_t now)
{
    ...
    task2Context.brakeStartTick = now;
...
void App_Task2Run(void)
{
    ...
    if (task2Context.phase == TASK2_BRAKING) {
        const uint32_t elapsed = now - task2Context.brakeStartTick;
        const float measuredSpeed = (Motor1_Speed + Motor2_Speed) * 0.5f;

        if ((elapsed >= APP_TASK2_SLOW_STOP_DURATION_MS) ||
    ...
    if ((task2Context.phase == TASK2_TRACKING) &&
        ...
        task2Context.phase = TASK2_BRAKING;
        task2Context.brakeStartTick = now;
    ...
    /* 刹车阶段继续循迹；速度PID公共输出反向，差速方向保持。 */
    if (output.valid != 0U) {
        App_VehicleClosedLoopSetTarget(output.leftTarget,
                                       output.rightTarget);
    } else if (task2Context.phase == TASK2_BRAKING) {
